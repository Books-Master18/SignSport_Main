...
def analyze_handwriting_goldberg(image_file):
    try:
        ...
        # ВАЖНО: Делаем массив непрерывным в памяти
        img = np.ascontiguousarray(img)
        
        # Простой порог вместо OTSU (OTSU использует внутренние фильтры, которые могут ломаться)
        ...
        # === 3. МЕЖСТРОЧНЫЙ ИНТЕРВАЛ (Через NumPy, без OpenCV фильтров) ===
        ...
        # === 8. РАЗМЕР И СКОРОСТЬ ===
        middle_zone_heights = [h for x, y, w, h in [cv2.boundingRect(c) for c in contours if cv2.boundingRect(c)[3] >= 10] 
                               if 0.33 <= ((y + h/2 - text_top) / text_height if text_height > 0 else 0.5) < 0.66]
        ...
        if zone_ratios["upper"] > 0.4: add_traits(ZONE_INTERPRETATION["upper"]["traits_if_developed"], 8)
        if zone_ratios["middle"] > 0.4: add_traits(ZONE_INTERPRETATION["middle"]["traits_if_balanced"], 10)
        if zone_ratios["lower"] > 0.4: add_traits(ZONE_INTERPRETATION["lower"]["traits_if_developed"], 7)
        ...
        add_traits(SIZE_INTERPRETATION[size_category]["traits"], 6)
        add_traits(SPEED_INTERPRETATION[speed_category]["traits"], 6)
        add_traits(SPACING_INTERPRETATION[spacing_category]["traits"], 5)
